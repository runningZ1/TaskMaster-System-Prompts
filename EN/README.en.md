...
此目录包含 Task Master 中所有 AI 驱动功能的集中式提示模板。

## 概述

提示管理系统提供：
- **集中存储**：所有提示都位于一个位置 (`/src/prompts`)
- **JSON Schema 验证**：使用 AJV 进行全面验证，并提供详细的错误报告
- **版本控制**：随时间跟踪提示的更改
- **变体支持**：针对不同上下文（研究模式、复杂性级别等）的不同提示
- **模板变量**：通过变量替换进行动态提示生成
...
│   └── variant.schema.json          # 提示变体模式
├── parse-prd.json          # PRD 解析提示
├── expand-task.json        # 任务扩展提示
├── add-task.json           # 任务创建提示
├── update-tasks.json       # 批量任务更新提示
├── update-task.json        # 单任务更新提示
├── update-subtask.json     # 子任务更新提示
├── analyze-complexity.json # 复杂性分析提示
└── research.json           # 研究查询提示
```

## Schema 验证

所有提示模板都将根据位于 `/src/prompts/schemas/` 中的 JSON 模式进行验证。验证系统：
...
- `useResearch` (boolean)：使用研究模式（默认值：false）
- `expansionPrompt` (string)：来自复杂性报告的扩展提示
...
- **增强的提示复杂性**：实现与 GitHub 规范匹配的 `parse-prd` 和 `expand-task` 逻辑
